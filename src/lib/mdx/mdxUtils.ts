...
export function getFiles() {
  return getFilesRecursive(postsDirectory);
}
...
export function getFileBySlug({ fileName, slug }: { fileName?: string; slug?: string }) {
  const files = getFilesRecursive(postsDirectory);
...
export function getAllBlogPostsWithCategories(): BlogPostsResponse {
  ...
  const allPostsData = fileNames.map((filePath) => {
    ...
    categories = [...categories, ...matterResult.data.genres];
    return {
      ...matterResult.data,
      fileName: path.basename(filePath),
      slug,
      imageUrl: matterResult.data.params.banner,
      avatar: matterResult.data.params.avatar,
      author: matterResult.data.params.author,
      category: matterResult.data.genres.join(","),
      tags: matterResult.data.tags.join(","),
      keywords: matterResult.data.keywords.join(","),
    } as BlogPostProps;
  });
