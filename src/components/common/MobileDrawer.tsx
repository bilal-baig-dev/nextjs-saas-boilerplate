...
type MobileDrawerProps = {
  isWaitlist: boolean;
  waitListNavMenu: NavItem[];
  navMenu: NavItem[];
};

const MobileDrawer = ({ isWaitlist, waitListNavMenu, navMenu }: MobileDrawerProps) => {
    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent side="left" className="bg-card">
        ...
        <div className="grid gap-3 text-sm py-6  mt-3">
          <NavMenu isSideBar menu={isWaitlist ? waitListNavMenu : navMenu} onNavLinkClick={handleNavLinkClick} />
        </div>
      </SheetContent>
    </Sheet>
  );
